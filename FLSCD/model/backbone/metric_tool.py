...
def export_outputs(outputs_dir, pred, id):

    img_name = id[0]
    pred_vis = pred * 255
    pred_vis = pred_vis.detach().cpu().numpy()

    pred_vis = pred_vis.astype('uint8').squeeze(0)
    #for batch_i in range(img_in1.shape[0]):
    output_path = os.path.join(outputs_dir, img_name)
    cv.imwrite(output_path, pred_vis)

    #print('export_outputs-processing is finished')
