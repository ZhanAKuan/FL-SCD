...
class  SemiCDDataset(Dataset):
    ...
    def __getitem__(self, item):
        if self.mode == 'val':
            ...
            id = self.ids[item].split('/')[-1]
            imgA_n = os.path.join(self.root, 'train', 'A', id)
            imgB_n = os.path.join(self.root, 'train', 'B', id)
            mask_n = os.path.join(self.root, 'train', 'label', id)


            imgA = Image.open(imgA_n).convert('RGB')
            imgB = Image.open(imgB_n).convert('RGB')

            mask = np.array(Image.open(mask_n))
            mask = mask / 255
            mask = Image.fromarray(mask.astype(np.uint8))
