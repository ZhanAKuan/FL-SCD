...
from model.semseg.deeplabv3plus_SSM_FRE import DeepLabV3Plus
import numpy as np
...
parser = argparse.ArgumentParser(description='FL-SCD')
...
def main():
    ...
    trainloader_l = DataLoader(trainset_l, batch_size=cfg['batch_size'],shuffle= False,
                               pin_memory=True, num_workers=2, drop_last=True, sampler=trainsampler_l)
    trainsampler_u = torch.utils.data.distributed.DistributedSampler(trainset_u)
    trainloader_u = DataLoader(trainset_u, batch_size=cfg['batch_size'],shuffle= False,
                               pin_memory=True, num_workers=2, drop_last=True, sampler=trainsampler_u)
    valsampler = torch.utils.data.distributed.DistributedSampler(valset)
    valloader = DataLoader(valset, batch_size=cfg['batch_size'], shuffle= False,pin_memory=True, num_workers=1,
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((imgA_x, imgB_x, mask_x),
            ...
            loss = loss_x * 0.8+ (loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.5 ) * 0.2
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, Mask ratio: {:.3f}' \
                                            .format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg,
                                            total_loss_w_fp.avg, total_mask_ratio.avg))
        
        iou_class, overall_acc, scores_dict = evaluate(model, valloader, cfg)


        if rank == 0:
            logger.info('***** Evaluation ***** >>>> Unchanged IoU: {:.2f}'.format(iou_class[0]))
            logger.info('***** Evaluation ***** >>>> Changed IoU: {:.2f}'.format(iou_class[1]))
            logger.info('***** Evaluation ***** >>>> Recall: {:.6f}'.format(scores_dict['recall_1']))
            logger.info('***** Evaluation ***** >>>> Precision: {:.6f}'.format(scores_dict['precision_1']))    
            logger.info('***** Evaluation ***** >>>> Unchanged F1: {:.6f}'.format(scores_dict['F1_0']))
            logger.info('***** Evaluation ***** >>>> Changed F1: {:.6f}'.format(scores_dict['F1_1']))

            logger.info('***** Evaluation ***** >>>> Mean IoU: {:.6f}\n'.format(scores_dict['miou']))
            logger.info('***** Evaluation ***** >>>> Mean F1: {:.6f}\n'.format(scores_dict['mf1']))
