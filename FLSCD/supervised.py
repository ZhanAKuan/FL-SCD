...
from model.backbone.metric_tool import ConfuseMatrixMeter
...
def evaluate(model, loader, cfg):
    ...
    with torch.no_grad():
        for imgA, imgB, mask, id in loader:
            ...
            current_score = running_metric.update_cm(pr=pred.cpu().numpy(), gt=mask.cpu().numpy())

            i = i + 1

            if (i % (len(loader) // 8) == 0) :
                print('Iters: {:} Done'.format(i))
    ...
    return iou_class, overall_acc, scores_dict
